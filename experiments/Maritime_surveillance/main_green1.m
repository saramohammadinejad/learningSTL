...
s2 = struct('name', 'y', ...
            ...
            'range', [15, 46]); 
signals = {s2,s1}; 
Traces = makeBreachTraceSystem(signals);
...
for k=1:200
    ...
    ro1= BrTrace.CheckSpec('alw_[0,60] (x[t] > 30)'); %testing the learned formula
    ro2=  BrTrace.CheckSpec('ev_[0,60] (y[t] < 22)'); %testing the learned formula
    yapp.addTrace(k,trace);
    if ro1>0
        plot(navalData(k,i),navalData(k,j),'b','LineWidth',0.5);
    elseif ro2>0
        plot(navalData(k,i),navalData(k,j),'r','LineWidth',0.5);
    else 
        plot(navalData(k,i),navalData(k,j),'g','LineWidth',0.5);
    end   
end

%% plotting threshold on the learned STL formulas
y=[];
y(1,1:81)=28.8430;
plot(0:80,y,'b--','LineWidth',2);

x=[];
x(1,1:31)=36.3260;
plot(x,15:45,'b--','LineWidth',2);

xlabel('x(t)');
ylabel('y(t)');
title('maritime surveilance case study');
...
clc
fprintf('running with optimization...it takes around 1 hour to run on a high speed platform\n')
tic
while (1)
...
pos = Traces1_test.CheckSpec('(y[t] > 19.7398) until_[0,49.2247] (x[t] < 24.8640)');
neg = Traces0_test.CheckSpec('(y[t] > 19.7398) until_[0,49.2247] (x[t] < 24.8640)');
